...
EVENT_URL = 'https://components.ifsc-climbing.org/results-api.php?api=event_results&event_id={}'
RESULT_URL = 'https://components.ifsc-climbing.org/results-api.php?api=event_full_results&result_url={}'

data = {'id': [], 'name': [], 'league_id': [], 
                'league_season_id': [], 'season_id': [], 
                'starts_at': [], 'ends_at': [], 'location': [], 
                'dcat_name': [], 'discipline_kind': [], 
                'category_name': [], 'rank': [], 'athlete_id': [], 
                'firstname': [], 'lastname': [], 
                'paraclimbing_sport_class': [], 
                'country': [], 'category_round_id': [], 
                'round_name': [], 'score': [], }
...
for i in tqdm.tqdm(range(50,steps), total=steps):
    try:
        ...
        for event in json_event['d_cats']:
            ...
            event_results = requests.get(RESULT_URL.format(event['full_results_url'])).json()
...
df.to_csv('../data/raw_data/climbing_data.csv', index=False)