...
DOTA_URL = 'https://www.gosugamers.net/dota2/matches/list/results?maxResults=100&page={}'
steps = 4500
    ...
    for i in tqdm.tqdm(range(0,steps), total=steps):
        ...
        for match in match_list.findChildren(recursive=False)[:-1]:
            ...
            if match.has_attr('class') and 'match-date' in match['class']:
                ...
                continue
            # get match info
            elif match.has_attr('class') and 'cell' in match['class'] and len(match['class']) == 1:
                ...
                tournament = info.find('div',{'class':'match-tournament'}).text.strip()
